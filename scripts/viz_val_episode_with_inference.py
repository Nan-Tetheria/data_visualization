...
def colors_future_gradient(n_pts):
    """Future: green -> red (near -> far). alpha=1."""
    if n_pts <= 0:
...
def main():
    ...
    ap.add_argument("--val_npz", type=str, default="/home/nan/datasets/deploy_data_1/val_data_episode_0000_eef_working_model.npz")
    ap.add_argument("--pred_npz", type=str, default="/home/nan/datasets/deploy_data_1/model_outputs_eef_working_model.npz")
    ...
    # --- Relative to wrist frame-0 (match your bag script style) ---
    t0 = t_w[0]
    R0T = R_w[0].T
    ...
    # Transform predicted wrist positions into rel frame-0 for 3D plotting
    pred_tw_rel = np.empty_like(pred_tw)
    for tt in range(T):
        pred_tw_rel[tt] = (R0T @ (pred_tw[tt] - t0).T).T  # (H,3)

    # For predicted future triads in 3D: also rotate into rel frame-0
    ...
    ax3d.set_title("Camera Center / Camera+0.1m (relative to wrist frame-0) + current/pred")
    ax3d.set_xlabel("x")
    ax3d.set_ylabel("y")
    ax3d.set_zlabel("z")

    wrist_line, = ax3d.plot(t_w_rel[:, 0], t_w_rel[:, 1], t_w_rel[:, 2], lw=2, color="lightgray", label="wrist")
    cam_center_line, = ax3d.plot(t_c_rel[:, 0], t_c_rel[:, 1], t_c_rel[:, 2], lw=2, color="black", label="camera center")
    cam_fwd_line, = ax3d.plot(t_cf_rel[:, 0], t_cf_rel[:, 1], t_cf_rel[:, 2], lw=2, color="purple", label="camera +0.1m")
    ax3d.legend(loc="upper left")

    # First frame marker + current wrist triad (RGB)
    ...
    # Predicted future polyline (rel frame-0)
    future_line, = ax3d.plot(pred_tw_rel[0, :, 0], pred_tw_rel[0, :, 1], pred_tw_rel[0, :, 2], linewidth=2.0)
    future_start = ax3d.scatter([pred_tw_rel[0, 0, 0]], [pred_tw_rel[0, 0, 1]], [pred_tw_rel[0, 0, 2]], s=25)
    ...
    for h in range(0, H, step):
        pred_triads.extend(draw_triad(ax3d, pred_tw_rel[0, h], pred_Rw_rel[0, h],
                                      axis_len=pred_axis_len, lw=pred_lw))

    # 3D bounds include wrist/cam/cam+0.1/pred
    all_pts = np.vstack([
        t_w_rel, t_c_rel, t_cf_rel,
        pred_tw_rel.reshape(-1, 3)
    ])
    ...
    # --- Right: image + projected predicted camera-forward points ---
    axim.set_title("2D: projected future (camera+0.1m), display=received, coords=original")
    axim.axis("off")

    # Show RECEIVED image (no rotation), but ORIGINAL is rotate180(received)
    img_received0 = np.transpose(imgs[0], (1, 2, 0))
    im_artist = axim.imshow(img_received0, vmin=0.0, vmax=1.0)

    future_sc = axim.scatter([], [], s=80, linewidths=0.0)
    gt_future_sc = axim.scatter([], [], s=80, linewidths=0.0)

    # principal point in DISPLAY coords
    ...
    def update(i):
        ...
        pred_triads.clear()
        for h in range(0, H, step):
            pred_triads.extend(draw_triad(ax3d, pred_tw_rel[i, h], pred_Rw_rel[i, h],
                                          axis_len=pred_axis_len, lw=pred_lw))

        # -------------------------
        # Update 2D image (received) + build ORIGINAL via rotate180
        # -------------------------
        ...
        # ORIGINAL image (for in-bounds in ORIGINAL coords)
        img_orig = rotate_image_180(img_received)
        h0, w0 = img_orig.shape[:2]

        # -------------------------
        # Project predicted future camera+0.1 points into CURRENT camera frame
        # Using rel frame: projection is invariant to the global rigid transform.
        # We'll compute future camera-forward points in rel frame:
        #   future wrist pose = pred_tw_rel[i,h], pred_Rw_rel[i,h]
        #   camera center = t + Rw*T_W_C
        #   camera rot    = Rw*R_w_c
        #   forward point = center + Rc*[0,0,0.1]
        # Then express in current camera frame i:
        #   P_cam = R_c_rel[i].T * (P_rel - t_c_rel[i])
        # -------------------------
        # Current camera (rel)
        tc_i = t_c_rel[i]
        Rc_i = R_c_rel[i]

        # Future camera-forward points (rel)
        # Build in vectorized form
        # t_c_fut_rel = pred_tw_rel + pred_Rw_rel @ T_W_C
        t_c_fut_rel = pred_tw_rel[i] + (pred_Rw_rel[i] @ T_W_C.reshape(3, 1)).reshape(H, 3)
        R_c_fut_rel = np.einsum("hij,jk->hik", pred_Rw_rel[i], R_w_c)
        t_cf_fut_rel = t_c_fut_rel + (R_c_fut_rel @ P_C_FORWARD.reshape(3, 1)).reshape(H, 3)

        dt = t_cf_fut_rel - tc_i.reshape(1, 3)       # (H,3)
        P_cam = (Rc_i.T @ dt.T).T                    # (H,3)
        ...
        if uv0.shape[0] > 0:
            # ORIGINAL -> DISPLAY (because display is received image)
            uv_disp = np.empty_like(uv0)
            uv_disp[:, 0] = (w_disp - 1) - uv0[:, 0]
            uv_disp[:, 1] = (h_disp - 1) - uv0[:, 1]
            future_sc.set_offsets(uv_disp)

            rgba = colors_future_gradient(uv_disp.shape[0])
        ...
        if i < j1:
            ...
            uv_gt = project_points_equidistant_full(
                P_cam_gt, FX, FY, CX, CY, K1, K2, K3, K4
            )

            inb0_gt = (
                (uv_gt[:, 0] >= 0) & (uv_gt[:, 0] < w0) &
                (uv_gt[:, 1] >= 0) & (uv_gt[:, 1] < h0)
            )
            ...
            if uv0_gt.shape[0] > 0:
                ...
                gt_future_sc.set_offsets(uv_disp_gt)

                rgba_gt = colors_gt_future_gradient(uv_disp_gt.shape[0])
                gt_future_sc.set_facecolors(rgba_gt)
                gt_future_sc.set_edgecolors(rgba_gt)
            else:
                gt_future_sc.set_offsets(np.empty((0, 2)))
        else:
            gt_future_sc.set_offsets(np.empty((0, 2)))
        ...
        return [wrist_line, cam_center_line, cam_fwd_line,
                ...
                im_artist, cam_cross, future_sc, gt_future_sc] + cur_triad + pred_triads
